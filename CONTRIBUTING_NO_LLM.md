...
All content submitted to this project's tracker and discussion spaces — including issue reports, feature requests, merge request descriptions, forum posts and all comments and responses — must be written by a human. Content generated by an LLM or autonomous agent, in whole or in part, is prohibited.

The use of LLMs is permitted only for grammar correction or translation of content originally authored by the contributor.

Maintainers reserve the right to close or remove submissions that violate these standards.
...
Examples:
```plain
Assisted-by: <model-name>-<version>
Assisted-by: <another-model-name>-<version>
```
